...
public class CustomVelocity extends ModuleMode<VelocityModule> {
    ...
    public CustomVelocity(String name, VelocityModule parent) {
        super(name, parent);
    ...
    EventCallback<PacketEvent> onPacket = event -> {
        if (mc.thePlayer != null) {
            if (event.getPacket() instanceof S12PacketEntityVelocity && ((S12PacketEntityVelocity) event.getPacket())
                ...
                packet.setMotionX((int) (packet.getMotionX() * (this.horizonal.getValue().floatValue() / 100f)));
                packet.setMotionY((int) (packet.getMotionY() * (this.vertical.getValue().floatValue() / 100f)));
                packet.setMotionZ((int) (packet.getMotionZ() * (this.horizonal.getValue().floatValue() / 100f)));

                if (this.horizonal.getValue().doubleValue() <= 0.0 && this.vertical.getValue().doubleValue() <= 0.0) {
                    event.cancel();
                }
            }
            ...
            if (event.getPacket() instanceof S27PacketExplosion) {
                ...
                packet.setField_149152_f((int) (packet.getField_149152_f()
                        * (this.horizonal.getValue().floatValue() / 100f)));
