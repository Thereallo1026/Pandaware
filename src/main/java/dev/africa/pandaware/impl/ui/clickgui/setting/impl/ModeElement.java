...
public class ModeElement extends Element<ModeSetting> {
    ...
    public void handleRender(Vec2i mousePosition, float pTicks) {
        ...
        this.width = (int) Fonts.getInstance().getProductSansMedium().getStringWidth(max.getName());
        ...
        Fonts.getInstance().getProductSansMedium().drawCenteredString(this.getSetting().getValue().getName(),
    ...
    ModuleMode<?> getLongest(ModeSetting setting) {
        return setting.getValues().stream()
                .reduce((a, b) -> setting.getName().length() > setting.getName().length() ? a : b)
                .orElse(null);
    }
}
